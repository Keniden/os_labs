...
void *thread_erosion(void *args)
{
    ...
    sem_post(&semaphore); // Сигнализируем о завершении работы потока
    pthread_exit(NULL);
}
...
int main(int argc, char *argv[])
{
    ...
    for (int k = 0; k < K; k++)
    {
        ...
        for (int t = 0; t < num_threads; t++)
        {
            ...
            sem_wait(&semaphore); // Ожидаем, чтобы начать выполнение потока
            if (pthread_create(&threads[t], NULL, thread_erosion, &targs[t]) != 0)
            {
                perror("Ошибка создания потока для эрозии");
                return 1;
            }
        }
        ...
        for (int t = 0; t < num_threads; t++)
        {
            ...
            {
                perror("Ошибка создания потока для наращивания");
                return 1;
            }
        }
        ...
        // Меняем матрицы для следующей итерации
        matrix_t *temp = A;
        A = C;
        C = temp;
    }
