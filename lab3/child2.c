...
int main()
{
    ...
    for (int i = 0; shm_ptr->upper_str[i] && i < MAX_LEN; i++)
    {
        if (isspace((unsigned char)shm_ptr->upper_str[i]))
        {
            shm_ptr->final_str[i] = '_';
        }
        else
        {
            shm_ptr->final_str[i] = shm_ptr->upper_str[i];
        }
    }

    shm_ptr->final_str[strlen(shm_ptr->upper_str)] = '\0';
